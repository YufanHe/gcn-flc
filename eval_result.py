...
parser.add_argument('--eval_file', dest='eval_file', default='./dataset/synthetic/dataset-04-28-09-26-04_output.json', help='hyperparameters')
parser.add_argument('--result_dir', dest='result_dir', default='./dataset/result/', help='hyperparameters')
parser.add_argument('--parallel_core', dest='parallel_core', default=3, help='hyperparameters')
...
def main():
    ...
    for i in range(N):
        p.apply_async(eval_multi, args=(data[i],data_save,lock)) 
...
def eval_multi(data_node,data_save,lock):
    """
    ...
    print("The optimal cost is %f, ther predicted cost is %f , ratio is %f" \
          % (gd_cost, pd_cost, pd_cost/gd_cost))
